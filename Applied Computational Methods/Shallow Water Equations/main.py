...
nc = 12
colorlevels = np.concatenate([np.linspace(-1, -0.05, nc), np.linspace(0.05, 1, nc)])
...
def plot_geo_adj(u, v, h):
    ...
    plt.ylim(-h0max * 0.12, h0max * 0.12)
...
for i in range(nsteps):
    ...
    if i % plot_interval == 0:
        plot(*state)
        print(
            "[t={:7.2f} u: [{:.3f}, {:.3f}], v: [{:.3f}, {:.3f}], h: [{:.3f}, {:.2f}]".format(
                t / 86400, u.min(), u.max(), v.min(), v.max(), h.min(), h.max()
            )
        )
